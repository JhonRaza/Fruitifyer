import { View, Text, StyleSheet, Dimensions, Image, TextInput, TouchableOpacity, ActivityIndicator, Button, Alert } from 'react-native'
  ...
  const userLogin = () => {
    if (email === '' && password === '') {
      ...
      auth()
        ...
        .catch(error => {setError(error.message), Alert.alert(error.message)})
    }
  }
    ...
    >
      ...
      {isPickerShow && (
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 7 }}>
          <TouchableOpacity onPress={userLogin} style={styles.SignUp}>
